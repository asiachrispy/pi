...
- Rulebook: a unified `Rule` shape loaded from `AGENTS.md` / `CLAUDE.md` (project + global), rendered as a `<rulebook>` block in the system prompt, and reachable from the model via `rule://root` (listing) and `rule://<name>` (single rule). Backed by `core/rules/` (parse, load, `rule-url`) and a new `ResourceLoader.getRules()` accessor. New `/rules` slash command lists loaded rules and supports `/rules <name>` to show one. `interruptMode` is parsed from frontmatter and reserved for the future time-traveling stream rules (TTSR) feature.
- TTSR (time-traveling stream rules): regex-based rules that abort an in-flight LLM stream the moment the assistant text matches, drop the bad partial, and inject the rule body as a `ttsr_injection` custom user message before retrying. The injection survives compaction because it is a first-class `custom_message` session entry. Configure with `tools.ttsr.{ enabled, rules[] }` in settings; rules from `AGENTS.md` with `interruptMode: always | prose-only | tool-only` are picked up automatically. A new `ttsr_triggered` extension event fires whenever a rule fires. The `rule://` URL scheme is a stage-1 prerequisite that this builds on.
- Handoff compaction strategy (`compaction.strategy: "handoff"`): generates a self-contained handoff document from the live system prompt, tool array, and conversation, then replaces the agent's message history with a single user message. The handoff document is persisted as a `custom_message` entry with `customType: "handoff"` so it survives session resume.
- Shell completions: `pi completions zsh | bash | fish` generates completion scripts from live flag metadata. Usage: `eval "$(pi completions zsh)"` in your shell rc file.
- `conflict://` URL scheme: the model writes `@ours`, `@theirs`, or `@base` to `conflict://<file>` and the write tool resolves the git conflict markers transparently.
- BM25 hidden tool discovery (`tools.discoveryMode: "bm25"`): a `search_tool_bm25` built-in tool lets the model search hidden tools by keyword and auto-activates matches for immediate use, avoiding prompt bloat.
- Tool approval modes (`tools.approvalMode: "yolo" | "write" | "always-ask"`, default `"yolo"`) with per-tool `tools.approval.<name>: allow | deny | prompt` overrides. Tools declare a `read` / `write` / `exec` tier via `ToolDefinition.approval`; the `bash` tool escalates critical patterns (`rm -rf /`, fork bombs, fetch-then-exec, writes to `/etc/passwd`, shutdown, `mkfs`/`dd` on `/dev/`) to `override: true` so they always prompt in non-yolo modes. New `--yolo` / `--auto-approve` CLI flag forces the session into `yolo` mode without persisting the change.
...
- Added a `memory.ts` example extension implementing project-local memory with compact index injection, branch-aware session events, `.pi/memory.jsonl` snapshots, memory tools, and `/memory` commands.
